/**
 * @file production-chooser-unique-quarter.js
 * @description 实现独特区域(UniqueQuarter)组件，用于在生产选择界面中显示文明特有的独特区域建筑
 */
...
export class UniqueQuarter {
    ...
    setBuildings(containerItemOne, containerItemTwo) {
        if (this.buildingElementOne == containerItemOne && this.buildingElementTwo == containerItemTwo) {
            return;
        }
        
        this.buildingContainer.innerHTML = '';
        this.buildingElementOne = containerItemOne;
    ...
    containsBuilding(item) {
        ...
        return this.buildingElementOne == item || this.buildingElementTwo == item ||
               (this.buildingElementOne && this.buildingElementOne.contains(item)) ||
