...
---------------
## 版本：1.9.5 (2026-02-05)
- 对官方v1.3.2兼容性更新，修复部分UI显示异常
- 修复：购买按钮错位到第二行的问题，恢复到生产条目的末尾处 (感谢 @ynoato 的反馈)
- 修复：购买单位时，会同时增加生产单位队列的异常 (感谢 @**ⓈHAⓇK** 的反馈)
- 修复：切换到购买模式，购买按钮不会隐藏的问题
- 优化：购买涨价类单位后，购买按钮上的价格将实时同步更新
- 修复：tooltip 提示框出现`Undefined`字符的情况
- 优化：tooltip 数值显示精度，由三位小数调整为一位小数
**暂未恢复的功能（因官方版本更新导致）**
- 暂失功能：紧凑型生产条目布局
- 暂失功能：生产条目内维护费用/生产力的显示
说明：官方新版`UI‑next`对生产条目采用动态渲染机制，原可直接引用的元素标识已移除，暂无法灵活调整相关布局与显示。
如需快速恢复功能可直接替换官方核心文件，但会丧失与其他模组的兼容性，导致其他模组对城市面板的修改全部失效。
为坚守本模组「最大兼容性」的核心原则，相关功能的完美修复仍需一定时间。

---------------
## 版本：1.9.4a (2026-01-19)
- 修复：多语言环境下小数点格式差异导致的城市产量栏数字显示异常，如俄语会显示`не число`。（感谢 @Zieha 的反馈）
- 性能优化
---------------
## 版本：1.9.4 (2026-01-19)
